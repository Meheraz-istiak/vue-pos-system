<template>
  <div class="md:flex md:space-x-5 email_height overflow-hidden relative rtl:space-x-reverse">
      ...
      <Card bodyClass=" h-full flex flex-col " className="h-full">
        ...
        <div class="flex-1 h-full">
          <div class="bg-white dark:bg-slate-900 rounded-md sticky top-0 mx-6 mt-6">
            <Button icon="heroicons-outline:plus" text="Module Group" btnClass="btn-dark w-full btn-sm block  "
              @click="toggleModal" />
          ...
          <div class="h-full pb-10 px-6" data-simplebar>
            <ul class="list ">
              ...
              <li v-for="item in moduleMenuData" :key="item.enptId" class="pt-2">
                ...
                <div class="input-group flex" @click="selectMenuItem(item.enptId)"
                  ...
                  <button class="px-2 py-3">
                    <span @click="module_editdata(item.enptId)" class="text-xl action-btn ">
                      <Icon icon="heroicons:pencil-square" />
                    </span>
                  </button>
                  <button>
                    <span @click="moduledeleteData(item.enptId)" class="text-xl action-btn ">
    ...
    <div class="flex-1 md:w-[calc(100%-320px)]" :class="width < 768 ? 'h-full' : ''">
      <Card bodyClass="p-0 h-full  relative" :className="width < 1024 ? 'h-full' : ''">
        <moduleTable :dropdownmenu="dropdownmenu" :pnamedata="pnamedata" :moduleByIdshowdata="moduleByIdshowdata" :moduleshowdata="moduleshowdata" />
  ...
  <template>
    <div>
      <Modal title="Module" label="Modal title" :activeModal="show" @close="show = false" themeClass="bg-primary-500">
        <div class="text-base text-slate-600 dark:text-slate-300">
          <Textinput label="Module Name" type="name" placeholder="Type Your Module Name" name="smallsize2"
            v-model="moduleData.name" />
          <Textinput label="Module Description" type="Description" placeholder="Enter Your Module Description "
            name="Description" v-model="moduleData.description" />
            <Textinput label="Module Icon" type="Icon" placeholder="Enter Your Module Icon "
            name="Description" v-model="moduleData.icon" />
        </div>
      ...
      <Modal title="Edit Module" label="Modal title" :activeModal="show1" @close="show1 = false" themeClass="bg-primary-500">
        <div class="text-base text-slate-600 dark:text-slate-300">
          ...
          <Textinput label="Module Description" type="Description" placeholder="Enter Your Module Description "
            ...
            <Textinput label="Module Icon" type="Icon" placeholder="Enter Your Module Icon "
            name="Description" v-model="updateData.icon" />
        </div>
...
async function showdata(id ,name) {
  ...
  const req = await axios.get("Module/GetModuleByGroupId/" + id)
  console.log('jhgjhgjh',req.data)
  dropdownmenu.value = res.data
...
const submitData = async () => {
  let res = await axios.post("Module/ModuleGroupInsert", moduleData.value)
  let moduleReq = res.data;
  console.log(res)
  if (res.status == 200) {
    show.value = false
    toast.success(moduleReq.mgs, { timeout: 1000 });
    ModuleData()
  } else {
    toast.error(moduleReq.mgs, { timeout: 1000 });
  }
  moduleData.value = {
    name: '',
    description: '',
...
let moduleMenuData = ref(null)
const ModuleData = async () => {
  let res = await axios.get("Module/GetModuleGroupList")
  // console.log(res.data)
  moduleMenuData.value = res.data
}
// Deleted module
const moduledeleteData = async (id) => {
      Swal.fire({
            title: 'Are you sure?',
          ...
          .then((response) => {
            if (response.isConfirmed) {
              ...
              const res = axios.delete('Module/ModuleGroupsDeleteById/'+id)
              .then((res)=>{
                if(res.status==200){
                  Swal.fire(
                'Deleted!',
...
async function module_editdata(id) {
  console.log('id',id)
  let res = await axios.get("Module/GetModuleGroupBy/" + id)
  if(res.data.id !== id){
  show1.value = true
...
const moduleUpdateData = async () => {
  ...
  let res = await axios.post("Module/EditModuleGroupById", updateData.value)
